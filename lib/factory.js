const factory = {
  PoolManager: async function ({ deployer }) {
    const factory = await ethers.getContractFactory('PoolManager', deployer);
  ...
  SchedulerMock: async function ({ deployer }) {
    const factory = await ethers.getContractFactory('SchedulerMock', deployer);
    const instance = await factory.deploy();
    return await instance.deployed();
  },
