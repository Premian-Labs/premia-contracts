...
contract PremiaOptionBatch {
    ...
    function batchWriteOption(
        IPremiaOption _premiaOption,
        IPremiaOption.OptionWriteArgs[] memory _options
    ) external {
    ...
    function batchExerciseOption(
        ...
        for (uint256 i = 0; i < _optionId.length; ++i) {
            _premiaOption.exerciseOptionFrom(
                msg.sender,
                _optionId[i],
                _amounts[i]
            );
    ...
    function batchWithdrawPreExpiration(
        IPremiaOption _premiaOption,
        uint256[] memory _optionId,
        uint256[] memory _amounts
    ) external {
        ...
        for (uint256 i = 0; i < _optionId.length; ++i) {
            _premiaOption.withdrawPreExpirationFrom(
                msg.sender,
                _optionId[i],
                _amounts[i]
            );
