...
contract Market is Ownable, ReentrancyGuard {
    ...
    function setFeeRecipient(address _feeRecipient) external onlyOwner {
        require(
            ...
            "FeeRecipient cannot be 0x0 address"
        );
    ...
    function addWhitelistedOptionContracts(address[] memory _addr)
        external
        onlyOwner
    {
    ...
    function removeWhitelistedOptionContracts(address[] memory _addr)
        external
        onlyOwner
    {
        MarketStorage.Layout storage l = MarketStorage.layout();
    ...
    /// @param _addr The list of addresses to add
    function addWhitelistedPaymentTokens(address[] memory _addr)
        external
        onlyOwner
    {
        MarketStorage.Layout storage l = MarketStorage.layout();
    ...
    /// @param _addr The list of addresses to remove
    function removeWhitelistedPaymentTokens(address[] memory _addr)
        external
        onlyOwner
    {
        MarketStorage.Layout storage l = MarketStorage.layout();
    ...
    function getAmountsBatch(bytes32[] memory _orderIds)
        external
        view
    ...
    function getOrderHashBatch(MarketStorage.Order[] memory _orders)
        external
        pure
    ...
    function getOrderHash(MarketStorage.Order memory _order)
        public
        pure
        returns (bytes32)
    {
    ...
    function getWhitelistedOptionContracts()
        external
        view
    ...
    function getWhitelistedPaymentTokens()
        external
        view
        returns (address[] memory)
    {
    ...
    function isOrderValid(MarketStorage.Order memory _order)
        public
        view
        returns (bool)
    {
        ...
        if (_order.side == MarketStorage.SaleSide.Buy) {
            ...
            uint256 basePrice = (_order.pricePerUnit * amountLeft) /
                (10**decimals);
            uint256 makerFee = IFeeCalculator(l.feeCalculator).getFee(
                _order.maker,
                IFeeCalculator.FeeType.Maker
            );
            ...
            bool isApproved = premiaOption.isApprovedForAll(
                _order.maker,
                address(this)
            );

            if (_order.isDelayedWriting) {
                IPremiaOption.OptionData memory data = premiaOption.optionData(
                    _order.optionId
                );
                IPremiaOption.OptionWriteArgs memory writeArgs = IPremiaOption
                .OptionWriteArgs({
                    token: data.token,
                    amount: amountLeft,
                ...
                IPremiaOption.QuoteWrite memory quote = premiaOption
                .getWriteQuote(_order.maker, writeArgs, _order.decimals);

                uint256 userBalance = IERC20(quote.collateralToken).balanceOf(
                    _order.maker
                );
                uint256 allowance = IERC20(quote.collateralToken).allowance(
                    _order.maker,
                ...
                uint256 optionBalance = premiaOption.balanceOf(
                    _order.maker,
                    _order.optionId
                );
    ...
    function areOrdersValid(MarketStorage.Order[] memory _orders)
        external
        view
        returns (bool[] memory)
    {
    ...
    function createOrder(MarketStorage.Order memory _order, uint256 _amount)
        public
        returns (bytes32)
    {
        bytes32 hash;
...

        {
            ...
            require(
                l.whitelistedOptionContracts.contains(_order.optionContract),
                "Option contract not whitelisted"
            );
            require(
                l.whitelistedPaymentTokens.contains(_order.paymentToken),
                "Payment token not whitelisted"
            );

            IPremiaOption.OptionData memory data = IPremiaOption(
                _order.optionContract
            ).optionData(_order.optionId);
            ...
            require(
                _order.isDelayedWriting == false || l.isDelayedWritingEnabled,
                "Delayed writing disabled"
            );
        }
    ...
    /// @return The hash of the order
    function createOrderForNewOption(
        MarketStorage.Order memory _order,
        uint256 _amount,
        MarketStorage.Option memory _option
    ) external returns (bytes32) {
        _order.optionId = IPremiaOption(_order.optionContract)
        .getOptionIdOrCreate(
            _option.token,
            _option.expiration,
            _option.strikePrice,
    ...
    function createOrders(
        MarketStorage.Order[] memory _orders,
        uint256[] memory _amounts
    ) external returns (bytes32[] memory) {
        require(
            _orders.length == _amounts.length,
    ...
    function createOrderAndTryToFill(
        MarketStorage.Order memory _order,
        uint256 _amount,
        MarketStorage.Order[] memory _orderCandidates,
        bool _writeOnBuyFill
    ) external {
        ...
        for (uint256 i = 0; i < _orderCandidates.length; i++) {
            require(
                _orderCandidates[i].side != _order.side,
                "Candidate order : Same order side"
            );
            require(
                _orderCandidates[i].optionContract == _order.optionContract,
                "Candidate order : Diff option contract"
            );
            require(
                _orderCandidates[i].optionId == _order.optionId,
                "Candidate order : Diff optionId"
            );
        }

        uint256 totalFilled;
        if (_orderCandidates.length == 1) {
            totalFilled = fillOrder(
                _orderCandidates[0],
                _amount,
                _writeOnBuyFill
            );
        } else if (_orderCandidates.length > 1) {
            totalFilled = fillOrders(
                _orderCandidates,
                _amount,
                _writeOnBuyFill
            );
    ...
    /// @param _order Order to create
    /// @return The hash of the order
    function writeAndCreateOrder(
        IPremiaOption.OptionWriteArgs memory _option,
        MarketStorage.Order memory _order
    ...
    function fillOrder(
...

        {
            ...
            if (
                _order.side == MarketStorage.SaleSide.Sell &&
                _order.isDelayedWriting
            ) {
                IPremiaOption(_order.optionContract).writeOptionWithIdFrom(
                    _order.maker,
                    _order.optionId,
                    _amount
                );
            } else if (
                _order.side == MarketStorage.SaleSide.Buy && _writeOnBuyFill
            ) {
                IPremiaOption(_order.optionContract).writeOptionWithIdFrom(
                    msg.sender,
                    _order.optionId,
            ...
            uint256 basePrice = (_order.pricePerUnit * _amount) /
                (10**_order.decimals);

            uint256 orderMakerFee = IFeeCalculator(l.feeCalculator)
            .getFeeAmount(
                _order.maker,
                basePrice,
                IFeeCalculator.FeeType.Maker
            );
            uint256 orderTakerFee = IFeeCalculator(l.feeCalculator)
            .getFeeAmount(msg.sender, basePrice, IFeeCalculator.FeeType.Taker);

            if (_order.side == MarketStorage.SaleSide.Buy) {
                IPremiaOption(_order.optionContract).safeTransferFrom(
                    msg.sender,
                    _order.maker,
                    _order.optionId,
                    _amount,
                ...
                IERC20(_order.paymentToken).safeTransferFrom(
                    _order.maker,
                    l.feeRecipient,
                    orderMakerFee + orderTakerFee
                );
                IERC20(_order.paymentToken).safeTransferFrom(
                    _order.maker,
                    msg.sender,
                    basePrice - orderTakerFee
                );
            } else {
                IERC20(_order.paymentToken).safeTransferFrom(
                    msg.sender,
    ...
    function fillOrders(
        ...
        uint256 takerFee = IFeeCalculator(l.feeCalculator).getFee(
            msg.sender,
            IFeeCalculator.FeeType.Taker
        );
        ...
        if (_orders.length > 1) {
            for (uint256 i = 0; i < _orders.length; i++) {
                require(
                    i == 0 ||
                        _orders[0].paymentToken == _orders[i].paymentToken,
                    "Different payment tokens"
                );
                require(
                    i == 0 || _orders[0].side == _orders[i].side,
                    "Different order side"
                );
                require(
                    i == 0 ||
                        _orders[0].optionContract == _orders[i].optionContract,
                    "Different option contract"
                );
                require(
                    i == 0 || _orders[0].optionId == _orders[i].optionId,
                    "Different option id"
                );
            }
        }
        ...
        for (uint256 i = 0; i < _orders.length; i++) {
            ...
            uint256 basePrice = (_order.pricePerUnit * amount) /
                (10**_order.decimals);
        ...
        if (_orders[0].side == MarketStorage.SaleSide.Buy) {
            ...
            IERC20(_orders[0].paymentToken).safeTransfer(
                l.feeRecipient,
                totalFee
            );
            ...
            // Batch payment of fees
            IERC20(_orders[0].paymentToken).safeTransferFrom(
                msg.sender,
                l.feeRecipient,
                totalFee
            );
